...
```
| パラメータ (colspan=2) | 値            | 必須 | デフォルト | 説明 |
|----------------------|---------------|------|-----------|------|
| (実際は↓のように表示) |               |      |           |      |
| パラメータ | Index    | 値            | 必須 | デフォルト | 説明 |
|-----------|----------|---------------|------|-----------|------|
| name      | (結合)   | example_name  | Yes  | -         | ... |
...
**ヘッダー行の説明**:
- `max_depth=1` なので、パラメータ関連の列は2列（パラメータ名列 + Index列）
- ヘッダーはこの2列を横方向に結合して「パラメータ」として表示（`colspan=2`）
...
```
| パラメータ (colspan=4)      | 値             | 必須 | デフォルト | 説明 |
|---------------------------|----------------|------|-----------|------|
| (実際は↓のように4列表示)   |                |      |           |      |
| パラメータ | Index | パラメータ | Index | 値             | 必須 | デフォルト | 説明 |
|-----------|-------|-----------|-------|----------------|------|-----------|------|
| name (colspan=4で横結合)   | nested_example | Yes  | -         | ... |
| rules     | 1     | name      | (結合) | rule_1         | No   | -         | ... |
|           |       | methods   | 1     | GET            | No   | -         | ... |
|           |       |           | 2     | POST           | No   | -         | ... |
|           |       |           | 3     | PUT            | No   | -         | ... |
|           |       | priority  | (結合) | 100            | No   | -         | ... |
...
|           |       | methods   | 1     | DELETE         | No   | -         | ... |
|           |       | priority  | (結合) | 200            | No   | -         | ... |
| enabled (colspan=4で横結合) | true           | No   | false     | ... |
```

**ヘッダー行の説明**:
- `max_depth=2` なので、パラメータ関連の列は4列（レベル1: パラメータ名+Index、レベル2: パラメータ名+Index）
- ヘッダーはこの4列を横方向に結合して「パラメータ」として表示（`colspan=4`）

**データ行の説明**:
- `name`と`enabled`は単純な属性なので、4つのパラメータ列を横結合（`colspan=4`）
- `rules`は配列なので、レベル1のパラメータ名（`rules`）とIndex（`1`, `2`）を使用
- `rules[0].methods`は2段階ネストなので、レベル2のパラメータ名（`methods`）とIndex（`1`, `2`, `3`）を使用
